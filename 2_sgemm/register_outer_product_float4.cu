...
__global__ void cuda_sgemm(float *A_ptr, float *B_ptr, float *C_ptr, const int M, const int N, const int K)
{
    ...
    float c_reg[M_NUM_PER_THREAD][N_NUM_PER_THREAD] = {0.f};

    // 3. K 维分块循环
    for (int s = 0; s < K; s += K_NUM_PER_BLOCK)
    {
        //每个线程负责搬运 16 个元素
        // --- 搬运 A 矩阵 (Global -> Shared) ---
        // 这里 A_ptr_start就是每个block块的起始位置了，现在线程坐标是（tx，ty）
        ...
        // 4. 计算逻辑 (Shared -> Register -> Temp)
        for (int k = 0; k < K_NUM_PER_BLOCK; k++)
        {
            // 加载 A 的一列到寄存器
            for (int i = 0; i < M_NUM_PER_THREAD; i++)
                a_reg[i] = a_shared[ty * M_NUM_PER_THREAD + i][k];
    ...
    // 5. 写回结果到 C 矩阵 (Register -> Global)
    float *C_ptr_start = C_ptr + blockIdx.y * M_NUM_PER_BLOCK * N + blockIdx.x * N_NUM_PER_BLOCK;
    
    for (int i = 0; i < M_NUM_PER_THREAD; i++) {
        FETCH_FLOAT4(C_ptr_start[(ty * M_NUM_PER_THREAD + i) * N + tx * N_NUM_PER_THREAD]) = FETCH_FLOAT4(c_reg[i][0]);
    }
}
